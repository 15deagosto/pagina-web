...
class Fn_43 {
    ...
    function fn43_rquejas_xagencia($LIMIT) {
        $con = new Conecciones;
        $mysqlidato = $con->crearConexion();
    ...
    function fn43_uquejas_estado($id, $estado) {
        $con = new Conecciones;
        $mysqlidato = $con->crearConexion();
    ...
    function fn43_tipo_quejas($tipos){
        $res="";
        if($tipos==1){
            $res="VENTANILLA";
        }
    ...
    function get_quejas_filto($fdesde,$fhasta, $agencia, $estado) {
        ...
        $sql2 = "select * from quejas q , nosotros n  WHERE q.id_gencia=n.id_nosotros And q.fecha_queja >= '$fdesde' and q.fecha_queja<='$fhasta' $txtfiltro1  ORDER by q.fecha_queja  DESC ";
        //echo $sql2;
        $arreglo;
        if (!$resultado2 = $mysqlidato->query($sql2)) {
    ...
    function fn43_rnosotros_all() {
        $con = new Conecciones;
        $mysqlidato = $con->crearConexion();
        $sql2 = "SELECT * FROM nosotros where estado_nosotros=1 ";
        $arreglo;
        if (!$resultado2 = $mysqlidato->query($sql2)) {
            echo "Error fn43_rnosotros_all -- fn43 ";
            exit;
        } else {
            $arreglo = $resultado2;
        }
