        ...
        <ul class="hidden lg:flex items-center gap-8 text-sm font-semibold">
            ...
            <li class="relative group">
                ...
                <ul class="absolute left-0 top-full mt-2 bg-white shadow-soft rounded-xl p-2 min-w-[220px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    <li><a href="microcredito.php?id=1" class="block px-3 py-2 rounded-lg hover:bg-rojo-light hover:text-rojo text-sm font-medium">Microcrédito</a></li>
                    <li><a href="consumo.php?id=1" class="block px-3 py-2 rounded-lg hover:bg-rojo-light hover:text-rojo text-sm font-medium">Consumo</a></li>
                    <?php
                    $detprod = $fnindex->fnindex_rproducto_xtipo(2);
                    while ($menudetprod = $detprod->fetch_assoc()) {
                        ?>
                        <li><a href="detprod-inversion-vencimiento.php?id=<?php echo $menudetprod['id_prod'] ?>" class="block px-3 py-2 rounded-lg hover:bg-rojo-light hover:text-rojo text-sm font-medium">Inversión: <?php echo utf8_encode($menudetprod['nombre_prod']) ?></a></li>
                        <?php
                    }
                    $detprod = $fnindex->fnindex_rproducto_xtipo(3);
                    while ($menudetprod = $detprod->fetch_assoc()) {
                        ?>
                        <li><a href="detprod-credito-1.php?id=<?php echo $menudetprod['id_prod'] ?>" class="block px-3 py-2 rounded-lg hover:bg-rojo-light hover:text-rojo text-sm font-medium">Ahorro: <?php echo utf8_encode($menudetprod['nombre_prod']) ?></a></li>
                        <?php
                    }
                    ?>
                </ul>
            ...
            <li class="relative group">
                <a href="nuestra-coop.php" class="hover:text-rojo transition-colors flex items-center gap-1">Conozcanos <i class="fa-solid fa-angle-down text-xs"></i></a>
                <ul class="absolute left-0 top-full mt-2 bg-white shadow-soft rounded-xl p-2 min-w-[220px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
